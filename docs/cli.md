...
Ошибки идут в stderr, код возврата `1` — ошибка, `2` — неверный вызов, `3` — `wait` не дождался письма.
...
строку-счётчик; `--all` показывает их, `--history` добавляет уже прочитанные заметки,
`--brief` — формат для хуков (пусто, когда нечего показать).

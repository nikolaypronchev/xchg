...
**Хук вместо опроса.** Агент узнаёт о сообщениях из хуков своего харнесса. Когда ничего нового нет,
хук печатает пустую строку — ноль байт в контексте и ноль токенов. Опрос по таймеру (цикл по
расписанию, cron) стоил бы хода модели на каждый тик, поэтому клиент его не предлагает и скилл прямо запрещает. Когда человека рядом нет, ту же роль играет `xchg wait`: хабы опрашивает процесс, а не модель, и агент просыпается только от настоящего письма.
...
**Клиент — один bash-скрипт.** Он должен работать везде, где у агента для программирования есть
оболочка, без установки чего бы то ни было: `bash`, `git`, `awk`, `sed`, coreutils. `python3` и `jq` — необязательные
